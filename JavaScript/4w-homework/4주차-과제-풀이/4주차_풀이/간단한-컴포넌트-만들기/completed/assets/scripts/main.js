...
prevButton.addEventListener('click', () => {
  // console.log('이전 슬라이드 탐색')
  goToCarouselList(-1)
})

nextButton.addEventListener('click', () => {
  // console.log('다음 슬라이드 탐색')
  goToCarouselList(1)
})
...
function goToCarouselList(direction) {
  // 현재 인덱스 값 수정(update)
  // 예) currentIndex = currentIndex + 1
  currentIndex = (currentIndex + direction + INDICATOR_TOTAL) % INDICATOR_TOTAL
  // console.log(currentIndex) // 0 -> 1 -> 2 -> 0 -> 1 -> 2 
  // 예) currentIndex = currentIndex + 1
  // currentIndex = currentIndex + direction
  // console.log(currentIndex) // 다음 탐색 시: 0 -> 1 -> 2 -> 0 -> 1 -> 2 
  // console.log(currentIndex) // 이전 탐색 시: 2 -> 1 -> 0 -> 2 -> 1 -> 0 
  // currentIndex = (0 + -1 + 3) % 3
  // console.log(currentIndex)

  // console.log('캐러셀 리스트(<ul>) 이동')
